...
public:
    ...
    void nextStep(double t)
    {
        ...
        if (vy > 0 && new_y > (WINDOW_Y - 50 - circle.getRadius())) //bounce along Y direction
        {
            vy = - (vy - ay*t); //reversing the direction of bubble
            new_y = circle.getY() + vy*t;
        }

        vy = vy + ay * t;   //updating velocity in Y
